...
file_count = "result_dict_新华字典.json"
xdhycd = "100年汉语新词新语大辞典_dictionary.json"
pre_dict_file = "all_corpus_standard.txt"
...
def get_frequency(str_input):
    ...
    return result
def add_new_value(str_input,key):

    str_input = str_input.strip().replace("。", "")
    if str_input in result_corpus_dict or str_input in pre_dict:
        return
    ...
    if len(str_input) <= 30:
        ...
        for i in range(0, len(str_input), 30):
            if key in str_input[i:i + 30]:
                result_corpus.append(str_input[i:i + 30])
                result_corpus_dict[str_input[i:i + 30]] = None
                for x in str_input[i:i + 30]:
                    if x in dict_list:
                        dict_list[x] += 1
...
pre_dict ={}
with open(pre_dict_file, 'r', encoding='utf-8') as file:
    content = file.readlines()
    for line in content:
        pre_dict[line.strip()] = None
...
# 输出到文件
with open("100年新词_corpus.txt", 'w', encoding='utf-8') as file:
    for x in result_corpus:
        file.write(x + "\n")
with open("result_dict_100年新词.json", 'w', encoding='utf-8') as file:
